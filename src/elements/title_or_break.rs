...
impl<'a, T: Element, C: Element> Element for TitleOrBreak<'a, T, C> {
    fn measure(&self, ctx: MeasureCtx) -> ElementSize {
        ...
        if let Some(breakable) = ctx.breakable {
            ...
            // Maybe the title should also be hidden if the first height is equal to the full
            // height?
    ...
    fn draw(&self, ctx: DrawCtx) -> ElementSize {
        ...
        self.size(title_size, content_size, show_title, break_count, collapse)
    }
}
...
impl<'a, T: Element, C: Element> TitleOrBreak<'a, T, C> {
    ...
    fn size(
        ...
        ElementSize {
            ...
            height: if collapse {
                None
            } else if break_count == 0 && show_title {
    ...
    fn test_pull_down() {
        ...
        .configurations()
        {
            let element = BuildElement(|BuildElementCtx { pass, .. }, callback| {
                ...
                let ret = callback.call(TitleOrBreak {
                    gap,
                    title: &title,
                    content: &content,
                ...
                content.assert_first_location_usage_count(if configuration.breakable {
                    1
                } else {
                    0
                });
                ...
                match pass {
                    ...
                    build_element::Pass::Draw { .. } => {
                        ...
                        content.assert_draw(DrawPass {
                            ...
                            first_height: if !configuration.breakable {
                                first_height - 3.
                            } else if configuration.use_first_height {
                                10.
                            } else {
                                7.
                            },
                ...
                match pass {
                    ...
                    build_element::Pass::Draw { .. } => {
                        ...
                        content.assert_draw(DrawPass {
                            width,
                            first_height: if configuration.breakable
                                && configuration.use_first_height
                            {
                                10.
                            } else {
                                first_height - 4.
                ...
                match pass {
                    ...
                    build_element::Pass::Draw { .. } => {
                        ...
                        content.assert_draw(DrawPass {
                            ...
                            page: if configuration.breakable { 1 } else { 0 },

                            layer: 0,
                            pos: (1., if configuration.breakable { 10. } else { 4. }),
                            breakable: if configuration.breakable {
                                Some(record_passes::BreakableDraw {
                                    full_height: 10.,
                                    preferred_height_break_count: 0,
